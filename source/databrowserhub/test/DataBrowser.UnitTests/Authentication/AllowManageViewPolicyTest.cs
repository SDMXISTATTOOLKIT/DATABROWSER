...
{
    public class AllowManageViewPolicyTest
    {
        private readonly Mock<IRequestContext> _requestContextMock;

        public AllowManageViewPolicyTest()
        {
        ...
        public async Task FilterNodeTrue_AuthActive_Allow()
        {
            ...
            var requirements = new[] {new AllowManageViewPolicy()};
            var user = new ClaimsPrincipal(
                new ClaimsIdentity(
                    new[]
                    {
                        new Claim("Permission_SingleNode_1", "ManageView")
                    },
                    "Basic")
            );

            var context = new AuthorizationHandlerContext(requirements, user, null);
            var subject = new AllowManageViewHandler(_requestContextMock.Object, _filterNodeMock.Object,
        ...
        public async Task FilterNodeFalse_AuthActive_Deny()
        {
            ...
            var user = new ClaimsPrincipal(
                new ClaimsIdentity(
                    new[]
                    {
                        new Claim("Permission_SingleNode_2", "ManageView")
                    },
                    "Basic")
            );

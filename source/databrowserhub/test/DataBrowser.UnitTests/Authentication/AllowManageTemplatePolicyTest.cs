...
{
    public class AllowManageTemplatePolicyTest
    {
        private readonly Mock<IRequestContext> _requestContextMock;

        public AllowManageTemplatePolicyTest()
        {
        ...
        public async Task FilterNodeTrue_AuthActive_Allow()
        {
            ...
            var requirements = new[] {new AllowManageTemplatePolicy()};
            var user = new ClaimsPrincipal(
                new ClaimsIdentity(
                    new[]
                    {
                        new Claim("Permission_SingleNode_1", "ManageTemplate")
                    },
                    "Basic")
            );

            var context = new AuthorizationHandlerContext(requirements, user, null);
            var subject = new AllowManageTemplateHandler(_requestContextMock.Object, _filterNodeMock.Object,
        ...
        public async Task FilterNodeFalse_AuthActive_Deny()
        {
            ...
            var user = new ClaimsPrincipal(
                new ClaimsIdentity(
                    new[]
                    {
                        new Claim("Permission_SingleNode_2", "ManageTemplate")
                    },
                    "Basic")
            );
